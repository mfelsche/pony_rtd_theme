...
This is a mobile-friendly Pony theme based on the sphinx_ theme David Snider made for readthedocs.org_.
...
.. code:: bash
    ...
    pip install pony_rtd_theme

In your ``conf.py`` file:

.. code:: python

    import pony_rtd_theme
    html_theme = "pony_rtd_theme"
    html_theme_path = [pony_rtd_theme.get_html_theme_path()]
...
Symlink or subtree the ``pony_rtd_theme/pony_rtd_theme`` repository into your documentation at
``docs/_themes/pony_rtd_theme`` then add the following two settings to your Sphinx
``conf.py`` file:
...
The theme's project-wide options are defined in the ``pony_rtd_theme/theme.conf``
...
* Add setuptools entry point allowing to use ``pony_rtd_theme`` as
...
.. _#215: https://github.com/rtfd/pony_rtd_theme/pull/215
...
The pony_rtd_theme is primarily a sass_ project that requires a few other sass libraries. I'm
...
#. Bump the version in ``pony_rtd_theme/__init__.py`` – we try to follow `semver <http://semver.org/>`_, so be careful with breaking changes.
